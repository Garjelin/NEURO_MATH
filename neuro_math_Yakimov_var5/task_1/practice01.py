...
Этот файл — только рабочее решение: минимизация и доп. задание, вывод в консоль и PNG.
Генерация DOCX выполняется отдельным скриптом (report_generator/), сюда не входит.
...
def coordinate_descent(
    ...
    # Для 2D это соответствует требованию «покоординатного» спуска из задания.
    x = x0.astype(float).copy()
    history = [f(x)]
    n = len(x)
...
def main() -> None:
    # Только вычисления и консоль + PNG. DOCX здесь не создаётся.
    data = run_practice01()
